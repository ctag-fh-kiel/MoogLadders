            ...
            for (int s = 0; s < n; ++s) {
                for (int stageIdx = 0; stageIdx < 4; ++stageIdx) {
                    if (stageIdx) {
                        ...
                        stage[stageIdx] = (h * stageZ1[stageIdx] + h0 * stageTanh[stageIdx - 1]) + (1.0 - g) *
                                                                                                   (stageIdx != 3
                                                                                                    ? stageTanh[stageIdx]
                                                                                                    : CTAG::SP::HELPERS::fasttanh(
                                                                                                                   stageZ1[stageIdx]));
                    } else {
                        input = samples[s] - ((4.0 * resonance) * (output - gainCompensation * samples[s]));
                        stage[stageIdx] = (h * CTAG::SP::HELPERS::fasttanh(input) + h0 * stageZ1[stageIdx]) +
